...
} from '#server/utils/bandwidth';

type Options = {
  enableIpv6?: boolean;
};

type ServerInterfaceOptions = Options & {
...
AllowedIPs = ${allowedIps.join(', ')}${extraLines.length ? `\n${extraLines.join('\n')}` : ''}`;
  ...
  generateServerInterface: (
    ...
    const { enableIpv6 = true, bandwidth, ifbAvailable = true } = options;
    ...
    // Combine hooks with tc commands (strip trailing semicolons to avoid ;;)
    const stripTrailingSemicolon = (s: string) => s.replace(/;+\s*$/, '');
    const finalPostUp = [iptablesTemplate(hooks.postUp, wgInterface), tcPostUp]
      .map((s) => stripTrailingSemicolon(s))
      .filter(Boolean)
      .join('; ');
    const finalPostDown = [
      tcPostDown,
      iptablesTemplate(hooks.postDown, wgInterface),
    ]
      .map((s) => stripTrailingSemicolon(s))
      .filter(Boolean)
      .join('; ');
